...
|---|---|---|
| traced baseline | none | every stock measurement in this tree, and the `baseline` column of the patched-arm table |
...
| all four | `apply-m5.py`, `apply-r1.py`, `apply-lazy.py`, `gate-export-keys.py` | both patched columns of the probe table |

The last row produces two columns because the export-key gate is inert unless
`DENO_LSP_SKIP_EXPORT_RESOLUTIONS` is set in the environment: unset, the binary
is M5 + R1 + the lazy dependency resolutions; set, it is that plus the skip. The
numbers are in
...
line to get the corresponding arm; omit all four for the traced baseline.
